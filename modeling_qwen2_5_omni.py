    ...
    def generate(
        ...
        **kwargs,
        ...
        if self.token2wav.dtype != torch.float:
            self.token2wav.float()
        wav = self.token2wav(talker_generate_codes.to(input_ids.device),
                conditioning=speaker_params["cond"].to(input_ids.device).float(),
                reference_mel=speaker_params["ref_mel"].to(input_ids.device).float(),
                **token2wav_kwargs)

        sf.write("./carl_test_sout_slice1.wav",wav.reshape(-1).detach().cpu().numpy(),samplerate=24000,)
        ...
        sf.write("./carl_test_sout_slice2.wav",wav.reshape(-1).detach().cpu().numpy(),samplerate=24000,)
        exit()
